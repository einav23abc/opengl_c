...
uvec2_t get_alert_box_pos(int32_t i) {
    ...
    if (alerts[i].box_pos_from_world_pos == 1) {
        vec2_t screen_cord = fbo_view_position_from_world_position(outport_fbo, alerts[i].box_world_pos_camera, vec3(alerts[i].box_world_pos_x, alerts[i].box_world_pos_y, alerts[i].box_world_pos_z));
        x = screen_cord.x;
        y = screen_cord.y;
    }
